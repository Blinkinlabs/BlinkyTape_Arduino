...
const uint8_t brightnesSteps[brightnessCount] = {255, 128, 64, 15, 7, 15, 64, 128};
...
//    TIMSK4 = _BV(TOV4);  // turn on the interrupt
    ...
    TCCR1B = _BV(WGM12) | _BV(CS12); // WGM13 WGM12 CS12 CS11 CS10
    TCCR1C = 0;
    OCR1AH = 0;     // Overflow here
    OCR1AL = 0x10;
    TCNT1H = 0;     // Reset the counter
    TCNT1L = 0;
    TIMSK1 = _BV(OCF1A); // Turn on the interrupt
    
  }
  else {
//    TIMSK4 = 0;  // turn off the interrupt
    TIMSK1 = 0; // Turn on the interrupt
  }
}
...
//  controller = &(LEDS.addLeds<WS2811, LED_OUT, GRB>(leds, ledCount));
  controller = &(LEDS.addLeds<DMXSIMPLE, LED_OUT, GBR>(leds, DEFAULT_LED_COUNT));
  LEDS.show(); 
}
